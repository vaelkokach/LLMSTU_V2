...
class ODVGDataset(BaseDetDataset):
    ...
    def __init__(self,
                 *args,
                 ...
                 clean_caption=True,
                 **kwargs) -> None:
        # May appear only in kwargs from cfg; must not reach mmengine BaseDataset
        # (raises TypeError: unexpected keyword argument 'label_map_file').
        _lmf = kwargs.pop('label_map_file', None)
        ...
        # Registry may duplicate ODVG-only keys in kwargs; never forward them.
        for _k in (
                'need_text',
                'actual_dataset_mode',
                'use_short_cap',
    ...
    def filter_data(self) -> List[dict]:
        """Respect ``filter_cfg['filter_empty_gt']`` (was ignored before)."""
        if self.test_mode:
        ...
        if not filter_empty_gt and min_size <= 0:
            return self.data_list
        valid: List[dict] = []
        for d in self.data_list:
            inst = d.get('instances') or []
            if filter_empty_gt and len(inst) == 0:
                continue
