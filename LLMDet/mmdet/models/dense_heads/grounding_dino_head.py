...
class GroundingDINOHead(DINOHead):
    ...
    def get_targets(self, cls_scores_list: List[Tensor],
        ...
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         pos_inds_list,
         neg_inds_list, assign_result_list) = multi_apply(self._get_targets_single,
                                      cls_scores_list, bbox_preds_list,
                                      batch_gt_instances, batch_img_metas)
        num_total_pos = sum((inds.numel() for inds in pos_inds_list))
        num_total_neg = sum((inds.numel() for inds in neg_inds_list))
        return (labels_list, label_weights_list, bbox_targets_list,
                bbox_weights_list, num_total_pos, num_total_neg, assign_result_list)
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        all_layers_outputs_classes, all_layers_outputs_coords = self(hidden_states, references, memory_text, text_token_mask)
        out_all_layers_outputs_classes = all_layers_outputs_classes.clone().detach()
        out_all_layers_outputs_coords = all_layers_outputs_coords.clone().detach()
        if self.use_dn:
            out_all_layers_outputs_classes = out_all_layers_outputs_classes[:, :, dn_meta['num_denoising_queries']:, :]
            out_all_layers_outputs_coords = out_all_layers_outputs_coords[:, :, dn_meta['num_denoising_queries']:, :]
            
        self.text_masks = text_token_mask
        loss_inputs = (all_layers_outputs_classes, all_layers_outputs_coords) + (enc_outputs_class, enc_outputs_coord,
                              batch_gt_instances, batch_img_metas, dn_meta)
        losses, all_stage_assign_result_list = self.loss_by_feat(*loss_inputs)
        return losses, all_stage_assign_result_list, out_all_layers_outputs_classes, out_all_layers_outputs_coords
    ...
    def loss_by_feat(
        ...
        if self.use_dn:
            ...
            all_layers_matching_cls_scores, all_layers_matching_bbox_preds = all_layers_cls_scores, all_layers_bbox_preds
            all_layers_denoising_cls_scores, all_layers_denoising_bbox_preds = None, None
        ...
        losses_cls, losses_bbox, losses_iou, all_stage_assign_result_list = multi_apply(
        ...
        return loss_dict, all_stage_assign_result_list
    ...
    def loss_by_feat_single(self, cls_scores: Tensor, bbox_preds: Tensor,
        ...
        with torch.no_grad():
            cls_reg_targets = self.get_targets(cls_scores_list,
                                               ...
                                               batch_img_metas)
        (labels_list, label_weights_list, bbox_targets_list, bbox_weights_list,
         num_total_pos, num_total_neg, assign_result_list) = cls_reg_targets
        ...
        return loss_cls, loss_bbox, loss_iou, assign_result_list
