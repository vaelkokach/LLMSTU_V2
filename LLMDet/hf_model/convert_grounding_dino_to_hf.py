...
#     self.bias = nn.Parameter(torch.Tensor([-math.log((1 - 0.01) / 0.01)]))
...
# GroundingDinoContrastiveEmbedding.forward = custom_forward
# GroundingDinoContrastiveEmbedding.bias = nn.Parameter(torch.Tensor([-math.log((1 - 0.01) / 0.01)]))
...
def get_grounding_dino_config(model_name):
    if "large" in model_name:
        ...
        config = GroundingDinoConfig(backbone_config=backbone_config, num_feature_levels=5, decoder_bbox_embed_share=False)
        return config
    ...
    config = GroundingDinoConfig(backbone_config=backbone_config, decoder_bbox_embed_share=False)
...
def create_rename_keys(state_dict, config):
    ...
    for layer, depth in enumerate(config.backbone_config.depths):
        for block in range(depth):
            ...
            rename_keys.append((f"backbone.0.layers.{layer}.blocks.{block}.attn.relative_position_index",
                            f"model.backbone.conv_encoder.model.encoder.layers.{layer}.blocks.{block}.attention.self.relative_position_index"))
    ...
    rename_keys.append(("transformer.decoder.class_embed.0.bias", "model.decoder.class_embed.0.bias"))
    rename_keys.append(("transformer.decoder.class_embed.1.bias", "model.decoder.class_embed.1.bias"))
    rename_keys.append(("transformer.decoder.class_embed.2.bias", "model.decoder.class_embed.2.bias"))
    rename_keys.append(("transformer.decoder.class_embed.3.bias", "model.decoder.class_embed.3.bias"))
    rename_keys.append(("transformer.decoder.class_embed.4.bias", "model.decoder.class_embed.4.bias"))
    rename_keys.append(("transformer.decoder.class_embed.5.bias", "model.decoder.class_embed.5.bias"))
    rename_keys.append(("transformer.enc_out_class_embed.bias", "model.encoder_output_class_embed.bias"))
...
def convert_grounding_dino_checkpoint(args):
    ...
    original_state_dict = torch.load(model_name, map_location="cpu")
    ...
    print(torch.all(new_state_dict['bbox_embed.4.layers.1.weight'] == new_state_dict['bbox_embed.5.layers.1.weight']))
    print(torch.all(new_state_dict['model.decoder.bbox_embed.4.layers.0.weight'] == new_state_dict['model.decoder.bbox_embed.5.layers.0.weight']))
